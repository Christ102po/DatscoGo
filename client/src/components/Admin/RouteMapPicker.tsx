  ...
  return (
    <section className="mt-4 overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
      <div className="border-b border-slate-200 bg-white px-4 py-3">
        ...
        <p className="mt-1 text-xs leading-5 text-slate-500">Choose the terminals, then click the map to add each barangay, junction, or stop in travel order. You can name every point and assign its fare below.</p>
        <div className="mt-3 flex flex-wrap gap-2">
          {([['origin', '1. Select origin'], ['destination', '2. Select destination'], ['waypoint', '3. Add barangays / stops']] as Array<[SelectionMode, string]>).map(([value, label]) => (
